...
export async function openTimerPopout(settings: FocusSettings) {
  ...
  await Promise.all([
    ...
    invoke("set_timer_size", { size: settings.popoutSize }),
  ]);
  if (settings.popoutRememberPosition && (!settings.popoutDockingEnabled || !settings.popoutDocked) && settings.popoutPositionX !== null && settings.popoutPositionY !== null) {
