...
export function TimerPage() {
  ...
  const dateControl = <span className="tooltip-host date-toggle-host"><button className={`date date-toggle ${todayOpen ? "active" : ""}`} aria-pressed={todayOpen} onClick={() => setTodayOpen((value) => !value)}>{dateTime || t("Today")}</button><span className="focus-tooltip" role="tooltip">{t(todayOpen ? "Hide Today pane" : "Show Today pane")}</span></span>;
  ...
  if (timer.state.running) return <main className={`timer-shell timer-shell--running ${todayOpen ? "timer-shell--today-open" : ""}`}><section className="running-stage" onPointerMove={trackFullscreenReveal} onPointerLeave={() => setFullscreenReveal(false)}>
