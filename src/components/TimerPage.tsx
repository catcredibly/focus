...
export function TimerPage() {
  ...
  if (timer.state.running) return <main className="timer-shell timer-shell--running"><section className="running-stage">
    ...
    <div className="running-labels"><span>Hours</span><span>Minutes</span><span>Seconds</span></div><div className="running-status">{timer.state.finished ? "Finished" : timer.state.paused ? "Paused" : ""}</div>
    <div className="running-controls"><button className="control-button" onClick={timer.state.finished ? timer.finish : timer.pause}>{timer.state.finished ? <Check size={25}/> : timer.state.paused ? <Play size={25} fill="currentColor"/> : <Pause size={25} fill="currentColor"/>}<span>{timer.state.finished ? "Finish" : timer.state.paused ? "Resume" : "Pause"}</span></button><button className="control-button" onClick={timer.stop}><Square size={22} fill="currentColor"/><span>Stop</span></button><div className="extend-wrap"><button className={`control-button ${extendOpen ? "control-button--accent" : ""}`} onClick={() => setExtendOpen((value) => !value)}><Plus size={27}/><span>Extend</span></button>{extendOpen && <TimerExtendMenu onClose={() => setExtendOpen(false)} onExtend={(seconds) => { timer.extend(seconds); setExtendOpen(false); }}/>}</div><button className="control-button" onClick={openPopout}><ExternalLink size={22}/><span>Pop out</span></button></div>
